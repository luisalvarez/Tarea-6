	...
	class mantenimientoSimple
	{
		...
		function __construct($tabla, $idRow)
		{
			...
			while($row = mysqli_fetch_array($rstabla))
			{
				...
				$this->gtitulo[0][0] = "Datos Generales";
					...
					switch($_GET["mod"])
					{
						case "consulta":
							?>
								...
								<br/>
								<div align="right">(doble click para editar) &nbsp;<button style="margin-left:100px;" type='button' onclick='newObjItemSimple()'>New</button></div>
									...
									<tbody id="busquedaBody<?php echo $this->tabla; ?>">
										<?php
											...
											while($row = mysqli_fetch_array($dadetalle))
											{
												...
												echo "<tr  class='$clase' ondblclick='AsFrmbuscarSimple(\"{$row[0]}\")'>";
													foreach ($this->visibleEnBusqueda as $campo)
													{
												...
												echo "<td><button style='cursor:pointer;' type='button' onclick='AsFrmbuscarSimple(\"{$row[0]}\")'>Edit</button></td></tr>";
												$c++;
												if($c++>3)
												{$c=1;}
											}
								...
								<div align="right"><button style="margin-left:100px;" type='button' onclick='newObjItemSimple()'>New</button></div>
				                </form>
								...
								<script language="javascript">
									...
									function AsFrmbuscarSimple(pagina)
									{ 
		...
		function display()
		{
				...
				<legend class='ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all'>
					...
					$_GET["id"] = $this->idRow;
							...
							$apagadoNuevos = ($vprimario > 0)?$this->apagadoNuevos:"disabled"; 
				...
				<script language='javascript'>
					...
					function buscarItemsSimple()
					{
						$('#tabMantenimiento$this->tabla').tabs('select', 0);
					
					}
					
					function eliminarSimple(cod)
					{
